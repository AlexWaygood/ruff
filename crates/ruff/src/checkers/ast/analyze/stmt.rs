    ...
    match stmt {
        ...
        Stmt::If(
            ...
            if checker.enabled(Rule::EmptyTypeCheckingBlock) {
                if typing::is_type_checking_block(stmt_if, &checker.semantic) {
                    flake8_type_checking::rules::empty_type_checking_block(checker, stmt_if);
                }
            }
            if checker.enabled(Rule::IfTuple) {
                pyflakes::rules::if_tuple(checker, stmt_if);
            }
            ...
            if checker.enabled(Rule::IfWithSameArms) {
                flake8_simplify::rules::if_with_same_arms(checker, checker.locator, stmt_if);
            }
            ...
            if checker.enabled(Rule::IfElseBlockInsteadOfDictLookup) {
                flake8_simplify::rules::manual_dict_lookup(checker, stmt_if);
            }
            ...
            if checker.enabled(Rule::IfElseBlockInsteadOfDictGet) {
                flake8_simplify::rules::use_dict_get_with_default(checker, stmt_if);
            }
            ...
            if checker.enabled(Rule::OutdatedVersionBlock) {
                pyupgrade::rules::outdated_version_block(checker, stmt_if);
            }
                ...
                if checker.any_enabled(&[
                        ...
                        .any(|scope| scope.kind.is_any_function())
                    {
                        ...
                        if checker.enabled(Rule::ComplexAssignmentInStub) {
                            flake8_pyi::rules::complex_assignment_in_stub(checker, stmt_assign);
                        }
