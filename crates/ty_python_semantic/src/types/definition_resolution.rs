...
/// Resolves a name's source definitions, including imported aliases and implicit builtins.
///
/// This lookup uses the semantic index, not the inferred type of a use-site expression.
/// In particular, numeric annotations resolve to the named class, not to every class
/// accepted by the annotation. The IDE wrapper handles that navigation-specific expansion.
...
///
/// The caller supplies the receiver type so this lookup does not infer the use-site scope.
