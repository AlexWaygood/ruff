...
impl AstNode for TypeParam {
    ...
    {
        match self {
            TypeParam::TypeVar(node) => node.visit_preorder(visitor),
            TypeParam::TypeVarTuple(node) => node.visit_preorder(visitor),
            TypeParam::ParamSpec(node) => node.visit_preorder(visitor),
        }
    }
}
