...
impl<'db> IntersectionBuilder<'db> {
    ...
    pub(crate) fn add_positive(mut self, ty: Type<'db>) -> Self {
        // Ensures that any normalizations done to unions are also performed on the intersection.
        // For example: `bool` -> `Literal[True] | Literal[False]`.
        let ty = UnionType::from_elements(self.db, [ty]);
...
impl<'db> InnerIntersectionBuilder<'db> {
    ...
    fn add_positive(&mut self, db: &'db dyn Db, new_positive: Type<'db>) {
        match new_positive {
            ...
            // Adding `object` to an intersection is a no-op.
            Type::Instance(InstanceType { class }) if class.is_known(db, KnownClass::Object) => {}
            _ => {
